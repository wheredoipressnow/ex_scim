defmodule ExScim.Operations.Groups do
  ...
  def get_group(id) do
    with {:ok, domain_group} <- Storage.get_group(id) do
      {:ok, Mapper.to_scim(domain_group)}
    end
  end

  def get_group_by_display_name(display_name) do
    with {:ok, domain_group} <- Storage.get_group_by_display_name(display_name) do
      {:ok, Mapper.to_scim(domain_group)}
    end
  end

  def get_group_by_external_id(external_id) do
    with {:ok, domain_group} <- Storage.get_group_by_external_id(external_id) do
      {:ok, Mapper.to_scim(domain_group)}
    end
  end

  def list_groups_scim(opts \\ %{}) do
    with {:ok, filter_ast} <- parse_filter(Map.get(opts, "filter")) do
      ...
      with {:ok, domain_groups, total} <-
        ...
        scim_groups = Enum.map(domain_groups, &Mapper.to_scim/1)
        {:ok, scim_groups, total}
      end
    end
  end

  def create_group_from_scim(scim_data) do
    with {:ok, schema_validated_data} <- Validator.validate_scim_schema(scim_data),
         ...
         {:ok, stored_group} <- Storage.create_group(data_with_metadata) do
  ...
  def patch_group_from_scim(group_id, scim_data) do
