defmodule ExScim.Users.Patcher do
  @moduledoc """
  Applies SCIM PatchOp operations to user data (maps or structs).
  ...
  def patch(user_data, %{"Operations" => operations}) when is_list(operations) do
    if length(operations) == 0 do
      ...
      try do
        updated =
          Enum.reduce(operations, user_data, fn op, acc ->
            apply_op(acc, op)
          end)
  ...
  defp apply_add(resource, %{"value" => value}) do
    # No path specified, merge at root
    deep_merge(resource, value)
  end
      ...
      try do
        update_in(resource, keys, fn
          nil when mode == :append ->
            # Check if this looks like a multi-valued field
  ...
  defp merge_into_struct(struct, map) when is_map(map) do
    Enum.reduce(map, struct, fn {key, value}, acc ->
      ...
      if atom_key && Map.has_key?(acc, atom_key) do
        ...
        # For non-existent fields, ignore them (don't add to struct)
        acc
      end
    end)
  end

  # Struct-aware update_in implementation
  ...
  defp update_in_struct(struct, [key], fun) when is_binary(key) do
    try do
      ...
      if Map.has_key?(struct, atom_key) do
        ...
        # Key exists as atom but not in struct, ignore
        struct
      end
    rescue
      # Key doesn't exist as atom, ignore
  ...
  # Struct-aware pop_in implementation
